    ...
    tabPanel(
      ...
      sidebarLayout(
        ...
        font-size: 16px;
          ...
          fluidRow( # style = "display: flex; justify-content: center; align-items: center;",
            column(
              12,
              # actionButton("calc_int", "Calculate Product Intensity", width = "95%")#Previous code
              column(12, uiOutput("dynamic_buttons"))
            )
          ),
          div(
...
server <- function(input, output, session) {
  ...
  results_data <- reactiveValues(
    ...
    all_products_breakdown = NULL,
    all_products_breakdown_new = NULL,
    all_products_summarized_baseline = NULL,
    all_products_breakdown_baseline = NULL,
    all_products_summarized_new = NULL,
  ...
  observeEvent(input$calc_int, {
    button_clicked(TRUE)
    show_results(TRUE)
    calc_type("baseline") # Setting to baseline calculation at first
    req(input$file)
    num <- input$products_num
    num_products(num)
    ...
    if (input$selected_method == "Quantity-based") {
      ...
      if (exists("all_products_breakdown")) {
        ...
        energy_summarized <- all_products_breakdown %>%
          ...
          group_by(product_name, energy_source) %>%
          summarise(
            by_source_energy_intensity = sum(qty_based_en_intensity, na.rm = TRUE),
            .groups = "drop"
          ) %>%
        ...
        for (i in 1:num) {
          ...
          product_part <- paste0(
            strong(product_name),
            ": ",
            tags$u(paste0(scales::comma(product_value, accuracy = 0.01), " ", input$energy_units_int, " energy")),
          ...
          energy_phrases <- if_else(
            ...
            paste0(
              energy_sources, "-based intensity is ",
      ...
      if (exists("all_products_breakdown")) {
        ...
        if (input$revenue_name == "Revenue %") {
          ...
          energy_summarized <- all_products_breakdown %>%
            ...
            group_by(product_name, energy_source) %>%
            summarise(
              by_source_energy_intensity = sum(revenue_based_energy_intensity_qty, na.rm = TRUE),
              .groups = "drop"
            ) %>%
            pivot_wider(names_from = "energy_source", values_from = "by_source_energy_intensity")
          ...
          energy_summarized <- all_products_breakdown %>%
            ...
            group_by(product_name, energy_source) %>%
            summarise(
              by_source_energy_intensity = sum(revenue_based_energy_intensity_dollar, na.rm = TRUE),
              .groups = "drop"
            ) %>%
            pivot_wider(names_from = "energy_source", values_from = "by_source_energy_intensity")
          ...
          for (i in 1:num) {
            ...
            energy_part <- if (length(clean_phrases) > 0) {
              paste0(
                clean_phrases[1],
                if (length(clean_phrases) > 1) {
                  paste0(", ", paste(clean_phrases[-1], collapse = ", "))
                } else {
                  ""
                }
              )
      ...
      tryCatch({
        if (input$ResultTabs == "Graphical Results") {
          ...
          if (calc_type() == "comparison") {
            ...
            breakdown_new <- results_data$all_products_breakdown_new
            summarized_new <- results_data$all_products_summarized_new
            
            req(!is.null(breakdown_new))
            req(!is.null(summarized_new))
            
            if (input$selected_method == "Quantity-based") {
              energy_summarized_new <- breakdown_new %>%
                mutate(product_name = factor(product_name, levels = unique(product_name))) %>%
                group_by(product_name, energy_source) %>%
                summarise(
                  by_source_energy_intensity = sum(qty_based_en_intensity, na.rm = TRUE),
            ...
            if (input$selected_method == "Quantity-based") {
              energy_costs_summarized_new <- breakdown_new %>%
                mutate(product_name = factor(product_name, levels = unique(product_name))) %>%
                group_by(product_name, energy_source) %>%
                summarise(
                  by_source_energy_costs_intensity = sum(energy_cost_based_intensity, na.rm = TRUE),
                  .groups = "drop"
                ) %>%
                pivot_wider(names_from = "energy_source", values_from = "by_source_energy_costs_intensity")
            } else if (input$revenue_name == "Revenue %") {
              energy_costs_summarized_new <- breakdown_new %>%
                ...
                group_by(product_name, energy_source) %>%
                summarise(
                  by_source_energy_costs_intensity = sum(revenue_based_energy_costs_intensity_qty, na.rm = TRUE),
                  .groups = "drop"
                ) %>%
                pivot_wider(names_from = "energy_source", values_from = "by_source_energy_costs_intensity")
            } else {
              energy_costs_summarized_new <- breakdown_new %>%
                ...
                group_by(product_name, energy_source) %>%
                summarise(
                  by_source_energy_costs_intensity = sum(revenue_based_energy_costs_intensity_dollar, na.rm = TRUE),
                  .groups = "drop"
                ) %>%
                pivot_wider(names_from = "energy_source", values_from = "by_source_energy_costs_intensity")
            }
            
            new_sentences_en_list <- vector("list", num)
            
            for (i in 1:num) {
              product_name <- summarized_new$product_name[i]
              unit_name <- input[[paste0("units_name_", i)]]
              energy_sources <- colnames(energy_summarized_new)[2:ncol(energy_summarized_new)]
              energy_values <- as.numeric(energy_summarized_new[i, 2:ncol(energy_summarized_new)])
              
              if (input$selected_method == "Quantity-based") {
                product_value <- summarized_new$qty_based_energy_intensity_mmbtu_ton[i]
                ...
                new_sentences_en_list[[i]] <- paste0(product_part, ". ", energy_part, ".")
                next
              } else if (input$revenue_name == "Revenue %") {
                product_value_qty <- summarized_new$revenue_based_energy_intensity_mmbtu_qty[i]
                ...
                product_value_dollar <- summarized_new$revenue_based_energy_intensity_mmbtu_dollar[i]
                product_value_qty <- summarized_new$revenue_based_energy_intensity_mmbtu_qty[i]
            ...
            new_sentences_en_0 <- format_sentences_bullet(new_sentences_en_list)
            new_sentences_en <- HTML(
              format_sentences(paste0("<br>", strong("Modified Scenario:"))),
              new_sentences_en_0
            )
            
            new_sentences_ec_list <- vector("list", num)
            
            for (i in 1:num) {
              product_name <- summarized_new$product_name[i]
              unit_name <- input[[paste0("units_name_", i)]]
              energy_sources <- colnames(energy_costs_summarized_new)[2:ncol(energy_costs_summarized_new)]
              energy_values <- as.numeric(energy_costs_summarized_new[i, 2:ncol(energy_costs_summarized_new)])
              
              if (input$selected_method == "Quantity-based") {
                product_value <- summarized_new$energy_costs_intensity_dollar_qty[i]
                ...
                energy_part <- paste(na.omit(energy_phrases), collapse = ", ")
                new_sentences_ec_list[[i]] <- paste0(product_part, ". ", energy_part, ".")
                next
              } else if (input$revenue_name == "Revenue %") {
                product_value <- summarized_new$energy_costs_intensity_revenue_based[i]
                ...
                product_value_dollar <- summarized_new$revenue_based_energy_costs_intensity_dollar[i]
                product_value_qty <- summarized_new$revenue_based_energy_costs_intensity_qty[i]
                
                product_part <- paste0(
                  strong(product_name), ": ",
                  tags$u(paste0("$", scales::comma(product_value_dollar, accuracy = 0.01))),
                  " in energy costs is required to generate a dollar in revenue"
                )
            ...
            new_sentences_ec_0 <- format_sentences_bullet(new_sentences_ec_list)
            new_sentences_ec <- HTML(
              format_sentences(paste0("<br>", strong("Modified Scenario:"))),
              new_sentences_ec_0
            )
            
            new_sentences_em <- format_sentences(c(
              paste0("<br>", strong("Modified Scenario:")),
              sapply(1:num, function(i) {
                product_name <- summarized_new$product_name[i]
                if (input$selected_method == "Quantity-based") {
                  product_value <- summarized_new$qty_based_emission_intensity_mtco2e_ton[i]
                  paste0(
                    strong(product_name), ": ",
                    tags$u(paste0(scales::comma(product_value, accuracy = 0.001), " metric ton of CO₂e")),
                    " is generated to produce ",
                    tags$u(paste0("one ", input[[paste0("units_name_", i)]]))
                  )
                } else if (input$revenue_name == "Revenue %") {
                  product_value <- summarized_new$revenue_based_emission_intensity_mtco2e_qty[i]
                  paste0(
                    strong(product_name), ": ",
                    tags$u(paste0(scales::comma(product_value, accuracy = 0.001), " metric ton of CO₂e")),
                    " is generated to produce ",
                  ...
                  product_value_dollar <- summarized_new$revenue_based_emission_intensity_mtco2e_dollar[i]
                  product_value_qty <- summarized_new$revenue_based_emission_intensity_mtco2e_qty[i]
                  paste0(
                    strong(product_name), ": ",
                    tags$u(paste0(scales::comma(product_value_dollar, accuracy = 0.001), " metric ton of CO₂e")),
                    " is generated per ", tags$u("dollar revenue")
                  )
                }
              })
            ))
          } # end if (calc_type() == "comparison")
          
          if (input$graph_tabs == "enPlotvalue") {
            HTML(paste(formatted_sentences_en, new_sentences_en, collapse = "<br>"))
          } else if (input$graph_tabs == "ecPlotvalue") {
            HTML(paste(formatted_sentences_ec, new_sentences_ec, collapse = "<br>"))
          } else if (input$graph_tabs == "emPlotvalue") {
            HTML(paste(formatted_sentences_em, new_sentences_em, collapse = "<br>"))
          } else {
        ...
        tagList(
          tags$p(
            style = "color: #636e72; font-size: 14px;",
            ""
          )
        )
      })
  ...
  observeEvent(input$calc_new_int, {
    ...
    if (input$selected_method == "Quantity-based") {
      ...
      aa_new <- aa_new %>%
        ...
        select(
          source,
          total_energy_new_mm_btu_yr,
          co2e_emissions_new_mt_co2e_yr,
          total_energy_costs_new_yr,
          energy_source
        ) %>%
        rename(
          total_energy_baseline_mm_btu_yr = total_energy_new_mm_btu_yr,
    ...
    # ==================== REVENUE-BASED METHOD ====================
      ...
      all_products_breakdown_baseline <- all_products_breakdown_baseline %>%
        mutate(
          qty_based_energy_costs = (qty_based_energy / total_energy_baseline_mm_btu_yr) * total_energy_costs_baseline_yr,
          revenue_based_energy_costs = (revenue_based_energy / total_energy_baseline_mm_btu_yr) * total_energy_costs_baseline_yr,
          qty_based_energy_costs_intensity = qty_based_energy_costs / product_qty,
          revenue_based_energy_costs_intensity_qty = revenue_based_energy_costs / product_qty
        ) %>%
        relocate(qty_based_energy_costs, .after = revenue_based_energy) %>%
        relocate(revenue_based_energy_costs, .after = qty_based_energy_costs)
      ...
      if (input$revenue_name == "Revenue %") {
        ...
        all_products_summarized_baseline <- all_products_breakdown_baseline %>%
          ...
          summarise(
            ...
            revenue_based_energy_intensity_mmbtu_qty = sum(revenue_based_energy_intensity_qty, na.rm = T),
            total_energy_costs_revenue_based = sum(revenue_based_energy_costs, na.rm = T),
            revenue_based_energy_costs_intensity_dollar = sum(revenue_based_energy_costs_intensity_dollar, na.rm = T),
            revenue_based_energy_costs_intensity_qty = sum(revenue_based_energy_costs_intensity_qty, na.rm = T),
            total_emissions_revenue_based_mtco2e_yr = sum(revenue_based_emissions, na.rm = T),
            revenue_based_emission_intensity_mtco2e_dollar = sum(revenue_based_em_intensity_dollar, na.rm = T),
            revenue_based_emission_intensity_mtco2e_qty = sum(revenue_based_em_intensity_qty, na.rm = T),
  ...
  output$enPlot <- renderPlotly({
    ...
    tryCatch({
      en_col <- if (input$selected_method == "Quantity-based") {
        "qty_based_en_intensity"
      } else if (input$revenue_name == "Revenue %") {
        "revenue_based_energy_intensity_qty"
      } else {
        "revenue_based_energy_intensity_dollar"
      }
      
      en_label <- if (input$selected_method == "Quantity-based") {
        paste0("Energy Intensity\n(", input$energy_units_int, "/unit of Product)")
      } else if (input$revenue_name == "Revenue %") {
        paste0("Energy Intensity\n(", input$energy_units_int, "/unit of Product)")
      } else {
        paste0("Energy Intensity\n(", input$energy_units_int, "/$ Revenue)")
      }
      
      if (calc_type() == "baseline") {
        all_products_breakdown_plot_data <- all_products_breakdown %>%
          group_by(product_name, energy_source) %>%
          summarise(value = sum(.data[[en_col]], na.rm = TRUE), .groups = "drop") %>%
          mutate(intensity_name = en_label) %>%
          na.omit()
        
        p <- ggplot(
          data = all_products_breakdown_plot_data,
          aes(
            x    = product_name,
            y    = value,
            fill = energy_source,
            text = paste0(energy_source, ": ", scales::comma(value, accuracy = 0.01), " ", input$energy_units_int)
          )
        ) +
  ...
  output$dynamic_buttons <- renderUI({
    ...
    if (show_second_button) {
      ...
      tagList(
        ...
        column(
          6,
          actionButton("calc_new_int", "Calculate Modified Product Intensity", width = "95%")
        )
      )
    } else {
      # Show only the original button
      actionButton("calc_int", "Calculate Product Intensity", width = "95%")
    }
  })
